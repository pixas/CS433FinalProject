    ...
    public:
        ...
        int load_batch_data(float *input_batch_images, std::vector<std::string>& image_names, int* output_batch_size) {
            ...
            for (i = 0; i < bsz; ++i) {
                ...
                cv::resize(temp, batched_images[i], cv::Size(224, 224));
                ...
                for (int c = 0; c < channels; ++c) {
                    for (int h = 0; h < height; ++h) {
                        for (int w = 0; w < width; ++w) {
                            input_batch_images[b * channels * height * width + c * height * width + h * width + w] = batched_images[b].at<cv::Vec3b>(h, w)[c];
                        }
                    }
                }
            }
